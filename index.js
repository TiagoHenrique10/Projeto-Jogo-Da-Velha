...
function initializeGame() {
    ...
    vBoard = [['','',''],['','',''],['','','']]
    turnPlayer = 'player1'
    // Ajuste o título da página (caso seja necessário)
    document.querySelector('h2').innerHTML = 'Vez de: <span id="turnPlayer"></span>'
    updateTitle()
    // Limpar o tabuleiro e adiconar os eventos de clique
...
function handleBoardClick(ev) {
    ...
    if(turnPlayer === 'player1') {
        span.innerText = 'X'
        span.classList.add('x-symbol')
        vBoard[row][column] = 'X'
    } else {
        span.innerText = 'O'
        span.classList.add('o-symbol')
        vBoard[row][column] = 'O'
    }
